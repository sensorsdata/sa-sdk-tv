...
public class SensorsDataAPI extends AbstractSensorsDataAPI {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (firstTrackInstallation) {
                            ...
                            // 先发送 track
                            trackEvent(EventType.TRACK, eventName, eventProperties, null, distinctId, loginId, null, null);
                            ...
                            if (mSAConfigOptions.mEnableMultipleChannelMatch) {
                                trackEvent(EventType.PROFILE_SET, null, profileProperties, null, distinctId, loginId, null, null);
                            } else {
                                trackEvent(EventType.PROFILE_SET_ONCE, null, profileProperties, null, distinctId, loginId, null, null);
                            }
        ...
        try {
            ...
            mTrackTaskManager.addTrackEventTask(new Runnable() {
                ...
                public void run() {
                    ...
                    trackEvent(EventType.TRACK, eventName, _properties, dynamicProperty, getDistinctId(), getLoginId(), null, null);
                }
            });
        } catch (Exception e) {
            SALog.printStackTrace(e);
        ...
        mTrackTaskManager.addTrackEventTask(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    DbAdapter.getInstance().deleteEventTimeInfo(eventName);
                } catch (Exception e) {
                    SALog.printStackTrace(e);
                }
            }
    ...
    public void trackTimerStart(String eventName) {
        trackTimerStart(eventName, null);
    }
            ...
            public void run() {
                try {
                    ...
                    if (eventTimeInfo != null) {
                        eventTimeInfo.distinctId = distinctId;
                        eventTimeInfo.loginId = getLoginId();
                        eventTimeInfo.anonymous_id = getAnonymousId();
                        eventTimeInfo.endTime = endTime;
                        trackTimerEndSync(eventTimeInfo, properties);
                    } else {
                        JSONObject _properties = ChannelUtils.checkOrSetChannelCallbackEvent(getConfigOptions().isAutoAddChannelCallbackEvent, eventName, properties, mContext);
    ...
    public void clearTrackTimer() {
        mTrackTaskManager.addTrackEventTask(new Runnable() {
            ...
            public void run() {
                try {
                    DbAdapter.getInstance().deleteAllEventTimeInfo();
                } catch (Exception e) {
                    SALog.printStackTrace(e);
                }
            }
