...
class SAProviderHelper {
    ...
    public void appendUri(UriMatcher uriMatcher, String authority) {
        try {
            ...
            uriMatcher.addURI(authority, DbParams.TABLE_EVENTS_TIMER, URI_CODE.EVENTS_TIMER);
            uriMatcher.addURI(authority, DbParams.TABLE_EVENTS_CACHE, URI_CODE.EVENTS_CACHE);
     ...
     * @return Uri
     */
    public Uri insertEvent(Uri uri, String tableName, ContentValues values) {
        try {
            SQLiteDatabase database = getWritableDatabase();
            ...
            long d = database.insert(tableName, "_id", values);
     ...
     * @return Uri
     */
    public Uri insertEventTimer(Uri uri, ContentValues values) {
        try {
            SQLiteDatabase database = getWritableDatabase();
            if (database == null) {
                return uri;
            }
            long d = database.insert(DbParams.TABLE_EVENTS_TIMER, DbParams.EVENTS_NAME, values);
            return ContentUris.withAppendedId(uri, d);
        } catch (Exception e) {
            SALog.printStackTrace(e);
        }
     ...
     * @return 受影响数
     */
    public int deleteEvents(int code, String selection, String[] selectionArgs) {
        if (!isDbWritable) {
            return 0;
        }
        int deletedCounts = 0;
        try {
            ...
            if (database != null) {
                switch (code) {
                    case URI_CODE.EVENTS:
                        deletedCounts = database.delete(DbParams.TABLE_EVENTS, selection, selectionArgs);
                        break;
                    case URI_CODE.EVENTS_TIMER:
                        deletedCounts = database.delete(DbParams.TABLE_EVENTS_TIMER, selection, selectionArgs);
                        break;
                    case URI_CODE.EVENTS_CACHE:
    ...
    public int updateEvent(String tableName, ContentValues values, String selection, String[] selectionArgs) {
        if (!isDbWritable) {
            return 0;
        }
        int rowCounts = 0;
        try {
            SQLiteDatabase liteDatabase = getWritableDatabase();
            if (liteDatabase != null) {
                rowCounts = liteDatabase.update(tableName, values, selection, selectionArgs);
            }
