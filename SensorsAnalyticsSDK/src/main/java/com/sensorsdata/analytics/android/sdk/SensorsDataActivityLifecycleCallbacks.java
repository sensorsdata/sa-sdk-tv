    ...
    private void trackAppEnd(long startTime, long pausedTime, String jsonEndData) {
        try {
            if (mSensorsDataInstance.isAutoTrackEnabled() && isAutoTrackAppEnd() && !TextUtils.isEmpty(jsonEndData)) {
                ...
                properties.put("$distinctId", endDataJsonObject.optString(DISTINCT_ID));
                properties.put("$loginId", endDataJsonObject.optString(LOGIN_ID));
                properties.put("$anonymous_id", endDataJsonObject.optString(ANONYMOUS_ID));
    ...
    private void generateAppEndData(long messageTime, long endElapsedTime) {
        try {
            ...
            if (SensorsDataAPI.getConfigOptions().isDataCollectEnable) {
                ...
                endDataProperty.put(DISTINCT_ID, mSensorsDataInstance.getDistinctId());
                endDataProperty.put(LOGIN_ID, mSensorsDataInstance.getLoginId());
                endDataProperty.put(ANONYMOUS_ID, mSensorsDataInstance.getAnonymousId());
                // 合并 $utm 信息
